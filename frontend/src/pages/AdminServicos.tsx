...
export default function AdminServicos() {
  ...
  async function handleSalvar(e: React.FormEvent) {
    e.preventDefault();
    const dados = { nome, preco, duracao_minutos: duracao };
    try {
      if (editando) {
        await axios.put(`http://localhost:8000/api/servicos/${editando}/`, dados, {
  ...
  return (
    <div className="min-h-screen bg-black text-white p-8">
      ...
      <form onSubmit={handleSalvar} className="max-w-md mx-auto mb-10">
        <input type="text" placeholder="Nome" className="w-full p-2 mb-2 bg-gray-800 rounded"
          value={nome} onChange={e => setNome(e.target.value)} required />
        <input type="text" placeholder="Preço" className="w-full p-2 mb-2 bg-gray-800 rounded"
          value={preco} onChange={e => setPreco(e.target.value)} required />
        <input type="number" placeholder="Duração (min)" className="w-full p-2 mb-4 bg-gray-800 rounded"
      ...
      <div className="grid gap-4 max-w-3xl mx-auto">
        {servicos.map(servico => (
          <div key={servico.id} className="bg-gray-800 p-4 rounded flex justify-between items-center">
            <div>
              <h2 className="font-bold">{servico.nome}</h2>
              <p className="text-gray-400">R$ {servico.preco} • {servico.duracao_minutos} min</p>
            </div>
            <div className="space-x-2">
              <button onClick={() => preencherEdicao(servico)} className="text-blue-400">Editar</button>
              <button onClick={() => handleExcluir(servico.id)} className="text-red-400">Excluir</button>
            </div>
          </div>
        ))}
      </div>
